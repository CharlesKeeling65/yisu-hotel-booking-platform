          ...
          <div className="content-title">
            ...
            <span className="title-actions">
              ...
              {isAdmin && Number(hotel.audit_status) === 0 ? (
                <button className="btn-mini btn-primary-mini" onClick={approve}>
                  审核通过
                </button>
              ) : null}
              {isAdmin && Number(hotel.audit_status) === 0 ? (
                <button className="btn-mini btn-danger-mini" onClick={reject}>
                  驳回
                </button>
              ) : null}
              {isAdmin &&
              ...
              Number(hotel.online_status) === 0 ? (
                <button className="btn-mini btn-primary-mini" onClick={publish}>
                  发布上线
                </button>
              ) : null}
              {Number(hotel.online_status) === 1 ||
              user?.role === "merchant" ? (
                <button className="btn-mini btn-danger-mini" onClick={offline}>
                  下线
                </button>
              ) : null}
            </span>
          ...
          <div className="hero-row">
            <div className="image-hero">
              {bannerImages.length ? (
                <img
                  ...
                  onError={(e) => {
                    e.currentTarget.src =
                      "https://picsum.photos/seed/yisu_web_hotel/1280/720";
                  }}
                />
              ) : (
            ...
            <div className="hotel-title-block">
              ...
              <div className="hotel-subline">
                开业时间：{formatCnDate(hotel.openTime)}
              </div>
                ...
                [...rooms]
                  ...
                  .map((r) => (
                    <tr key={r.id}>
                      <td className="room-thumb">
                        {r.image ? (
                          <img
                            ...
                            onError={(e) => {
                              e.currentTarget.src =
                                "https://picsum.photos/seed/yisu_web_room/800/500";
                            }}
                          />
                        ) : (
                          <div className="thumb-empty">无图</div>
                        )}
                      </td>
                      <td>{r.type}</td>
                      <td>{r.occupancy}人</td>
                      <td>{r.size || "-"}㎡</td>
                      <td>
                        <span className={`room-status ${r.status}`}>
                          {r.status === "soldout" ? "已售罄" : "可预订"}
                        </span>
                      </td>
                      <td>
                        <span className="price-current">{r.current}元</span>
                      </td>
                    </tr>
                  ))
              ) : (
