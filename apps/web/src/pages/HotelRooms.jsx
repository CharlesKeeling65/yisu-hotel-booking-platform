import { AlertCircle, ArrowLeft, CheckCircle2, Image as ImageIcon, Plus, Save, Trash2, UploadCloud, Building2, UserCircle, LogOut } from 'lucide-react';
...
const EMPTY_ROW = {
  type: '', area: '', capacity: '', original: '', current: '',
  discount: '', remain: '', status: 'available', remark: '', image: ''
};
...
export default function HotelRooms() {
  ...
  const mergeLocalExtras = (apiRooms) => {
    try {
      const localExtras = JSON.parse(localStorage.getItem(`roomExtras:${hotelId}`) || '{}');
  ...
  const persistExtrasToLocal = (roomsList) => {
    ...
    const extras = {};
    roomsList.forEach(r => {
      const payload = { discount: r.discount, area: r.area, capacity: r.capacity, remark: r.remark };
    ...
    if (isDirty) {
      setConfirmModal({
        ...
        onConfirm: async () => {
          ...
          if (success) { setTimeout(() => navigate(targetPath), 800); }
        }
      });
    } else {
      navigate(targetPath);
  ...
  async function saveAll() {
    ...
    try {
      ...
      const res = await fetch(`/api/hotels/${hotelId}/rooms/bulk-save`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ rooms: payloadRooms })
      });
      ...
      console.error(err);
      showToast(err && err.message ? err.message : '同步失败，请检查网络或服务端', 'error');
      return false;
    }
  }
  ...
  function handleFieldChange(idx, field, value) {
    ...
    if (field === 'original' || field === 'discount') {
      const newOriginal = field === 'original' ? toFiniteNumber(value) : toFiniteNumber(row.original);
      const newDiscount = field === 'discount' ? String(value) : String(row.discount || '');
  ...
  const inputClass = "w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-[14px] text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500/40 focus:border-blue-500 focus:bg-white transition-all";
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-24">
        ...
        <div className="custom-modal-overlay">
          <div className="custom-modal">
            <div className="modal-header">
              ...
              <button className="close-btn" onClick={() => { if (confirmModal.onCancel) confirmModal.onCancel(); setConfirmModal(null); }}><X size={22} /></button>
            </div>
            <p style={{ marginBottom: '28px', whiteSpace: 'pre-wrap', lineHeight: '1.6', color: '#47586e', fontSize: '15px' }}>
              {confirmModal.msg}
            </p>
            <div className="modal-actions">
              <button className="btn-modal cancel text-[15px]" onClick={() => { if (confirmModal.onCancel) confirmModal.onCancel(); setConfirmModal(null); }}>
        ...
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-slate-800 to-slate-600 tracking-tight">
              易宿平台 <span className="font-normal text-slate-400 mx-2">|</span> 房型库存管理
            </span>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-6 mt-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8">
          <div>
            <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight mb-2">房型与库存设置</h1>
            <p className="text-[15px] text-slate-500">{hotelId && hotelId !== '0' ? `当前管理资产 ID: ${hotelId}` : '当前为新资产草稿状态'}</p>
          </div>
          
          <div className="flex flex-wrap items-center gap-3">
            <button onClick={() => handleSafeBack(hotelId === '0' ? '/dashboard' : `/hotels/${hotelId}`)} className="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-slate-200 text-slate-600 hover:text-blue-600 hover:bg-blue-50 rounded-xl text-[15px] font-semibold shadow-sm transition-colors">
              <ArrowLeft size={16} /> 返回资产页
            </button>
            <button onClick={clearAll} className="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-slate-200 text-rose-500 hover:bg-rose-50 hover:border-rose-200 rounded-xl text-[15px] font-semibold shadow-sm transition-colors">
              <Trash2 size={16} /> 清空草稿
            </button>
            <button onClick={addRow} className="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-50 border border-blue-100 text-blue-700 hover:bg-blue-100 rounded-xl text-[15px] font-semibold transition-colors">
              <Plus size={16} /> 新增房型
            </button>
            <button onClick={saveAll} className="inline-flex items-center gap-2 px-6 py-2.5 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-[15px] font-bold shadow-md transition-colors">
              <Save size={16} /> 全部保存生效
            </button>
          </div>
        </div>
        ...
        <div className="space-y-6" ref={listRef}>
          {rows.map((r, idx) => (
            <div 
              ...
              className={cn(
                ...
                highlightIndex === idx ? 'border-blue-400 ring-4 ring-blue-500/10 shadow-lg' : 'border-slate-200/70 hover:shadow-md hover:border-blue-200'
              )}
            >
              ...
              <div className="w-full md:w-64 shrink-0 flex flex-col gap-3">
                <div className="relative aspect-[4/3] rounded-2xl overflow-hidden bg-slate-100 border border-slate-200/60 shadow-sm group">
                  {r.image ? (
                    <img src={r.image} alt="房型配图" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                  ) : (
                ...
                <label className="cursor-pointer inline-flex items-center justify-center gap-2 w-full px-4 py-2.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-xl text-[14px] font-semibold transition-colors border border-blue-100">
                  ...
                  <input type="file" className="hidden" accept="image/*" onChange={e => { const f = e.target.files && e.target.files[0]; if (f) handleImgFile(f, idx); e.target.value = ''; }} />
                </label>
              </div>
              ...
              <div className="flex-1 flex flex-col">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-5">
                  ...
                  <div>
                    ...
                    <input className={cn(inputClass, "type font-bold text-slate-900")} placeholder="如：高级大床房" value={r.type || ''} onChange={e => updateRow(idx, { type: e.target.value })} />
                  </div>
                  <div>
                    <label className={labelClass}>房型面积</label>
                    <input className={inputClass} placeholder="如：30" value={r.area || ''} onChange={e => updateRow(idx, { area: e.target.value })} />
                  </div>
                  <div>
                    <label className={labelClass}>可入住人数</label>
                    <input type="number" className={inputClass} placeholder="如：2" value={r.capacity || ''} onChange={e => updateRow(idx, { capacity: e.target.value })} />
                  </div>
                  <div>
                    <label className={labelClass}>实际剩余房量 (间)</label>
                    <input type="number" className={inputClass} placeholder="0" value={r.remain || ''} onChange={e => handleFieldChange(idx, 'remain', e.target.value)} />
                  </div>

                  {/* 第二排 */}
                  <div>
                    <label className={labelClass}>日常原价 (¥)</label>
                    <input type="number" className={inputClass} placeholder="0.00" value={r.original || ''} onChange={e => handleFieldChange(idx, 'original', e.target.value)} />
                  </div>
                  <div>
                    <label className={labelClass}>房型折扣</label>
                    <input className={inputClass} placeholder="如：8折或85折" value={r.discount || ''} onChange={e => handleFieldChange(idx, 'discount', e.target.value)} />
                  </div>
                  <div>
                    <label className={labelClass}>今日挂牌价 (¥)</label>
                    <input type="number" className={cn(inputClass, "text-blue-600 font-bold bg-blue-50/40 border-blue-100")} placeholder="自动计算或手填" value={r.current || ''} onChange={e => updateRow(idx, { current: e.target.value })} />
                  </div>
                  <div>
                    <label className={labelClass}>预订状态</label>
                    {/* ====== 去除下拉箭头，统一成普通输入框的样式，禁用点击事件 ====== */}
                    <div className={cn(inputClass, "flex items-center font-medium pointer-events-none")}>
                      {r.status === 'soldout' ? '🔴 满房售罄' : r.status === 'low' ? '🟡 房量紧张' : '🟢 开放预订'}
                    </div>
                  </div>
                  ...
                  <div className="sm:col-span-2 lg:col-span-4">
                    ...
                    <input className={inputClass} placeholder="如：包含双人自助早餐、免费接机、高楼层无窗等特殊说明..." value={r.remark || ''} onChange={e => updateRow(idx, { remark: e.target.value })} />
                  </div>
                </div>

                <div className="mt-auto pt-6 flex items-center justify-end gap-3">
                  <button type="button" onClick={(e) => { e.stopPropagation(); deleteRow(idx); }} className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-white border border-rose-200 text-rose-500 hover:text-rose-600 hover:bg-rose-50 hover:border-rose-300 rounded-xl text-[14px] font-semibold shadow-sm transition-all">
                    <Trash2 size={16} /> 删除房型
                  </button>
                  <button type="button" onClick={(e) => { e.stopPropagation(); saveRowToServer(idx); }} className="inline-flex items-center gap-1.5 px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-[14px] font-semibold shadow-md shadow-blue-500/20 transition-all">
                    <Save size={16} /> 保存此房型
                  </button>
                </div>
              </div>
          ...
          {rows.length === 0 && (
            <div className="py-24 flex flex-col items-center justify-center text-center bg-white rounded-3xl border border-dashed border-slate-300 shadow-sm">
              ...
              <h3 className="text-lg font-bold text-slate-700 mb-1">暂无任何房型数据</h3>
              <p className="text-[15px] text-slate-500 mb-6">您需要至少添加一个房型，酒店才能正常展示与售卖。</p>
              <button onClick={addRow} className="inline-flex items-center gap-2 px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-[15px] font-bold shadow-md transition-colors">
                <Plus size={18} /> 立即新增房型
              </button>
            </div>
          )}
