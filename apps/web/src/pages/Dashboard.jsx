...
export default function Dashboard() {
  ...
  async function loadHotels() {
    ...
    try {
      ...
      if (statusFilter === "approvedOffline") {
        params.set("auditStatus", "1");
        params.set("onlineStatus", "0");
      }
      if (statusFilter === "online") {
        params.set("auditStatus", "1");
        params.set("onlineStatus", "1");
      }
    ...
    if (action === "reject") {
      setPromptModal({
        ...
        onConfirm: async (reason) => {
          ...
          await executeAudit(hotelId, action, reason);
          return true;
        },
      });
    } else {
  ...
  async function publishHotel(hotelId) {
    const res = await fetch(`/api/admin/hotels/${hotelId}/publish`, {
      method: "POST",
    });
  ...
  async function offlineHotel(hotelId) {
    const res = await fetch(
      isAdmin
        ? `/api/admin/hotels/${hotelId}/offline`
        : `/api/hotels/${hotelId}`,
  ...
  const cover = (h) =>
    Array.isArray(h.images) && h.images.length ? h.images[0] : h.image || "";
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-12">
      ...
      <main className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          {isMerchant && (
            <button
              onClick={() => navigate("/hotels/0/edit")}
              className="inline-flex items-center justify-center gap-2 px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-xl shadow-lg shadow-blue-600/20 transition-all hover:shadow-blue-600/30 hover:-translate-y-0.5 active:translate-y-0"
            >
              <Plus size={18} /> 新增资产
            </button>
          )}
        ...
        <div className="bg-white p-2 rounded-2xl shadow-sm border border-slate-200/60 mb-8 flex flex-col xl:flex-row gap-2">
          ...
          <div className="flex items-center gap-2 p-2">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-sm font-semibold rounded-xl shadow-md transition-colors whitespace-nowrap"
            >
              {loading ? "检索中..." : "查 询"}
            </button>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {hotels.map((h) => {
                ...
                return (
                  <div
                    key={h.id}
                    className="group bg-white rounded-2xl border border-slate-200/70 overflow-hidden hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 flex flex-col hover:-translate-y-1"
                  >
                    <div
                    ...
                    >
                      ...
                      <div className="absolute top-3 right-3">
                        <span
                          className={`inline-flex px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm ring-1 ${meta.bg} ${meta.text} ${meta.ring} backdrop-blur-md`}
                        >
                          {meta.label}
                        </span>
                      </div>
                    ...
                    <div className="p-5 flex-1 flex flex-col min-w-0">
                      ...
                      <p className="text-xs text-slate-400 font-medium mb-4 truncate font-mono">
                        {h.nameEn || "No English Name"}
                      </p>
                      ...
                      <div className="space-y-2.5 mb-auto overflow-hidden">
                        <div className="flex items-center gap-2 text-[13px] text-slate-600">
                          ...
                          <span
                            ...
                            title={`${h.city}${h.county}${h.address}`}
                          >
                            {h.city}
                            {h.county}
                            {h.address}
                        ...
                        {isAdmin && (
                          <div className="flex items-center gap-2 text-[13px] text-blue-700">
                            ...
                            <span
                              ...
                              title={h.merchantName || h.merchantEmail}
                            >
                              {h.merchantName ||
                                h.merchantEmail ||
                                `ID: ${h.merchantId || "未知"}`}
                            ...
                            <Popover>
                              <PopoverTrigger asChild>
                                <button className="w-full flex items-center justify-between px-3 py-2 bg-rose-50/60 hover:bg-rose-100/60 border border-dashed border-rose-200 rounded-lg text-rose-600 transition-colors focus:outline-none group/reject">
                                  ...
                                  <span className="text-[11px] font-semibold bg-white px-2 py-0.5 rounded shadow-sm border border-rose-100 group-hover/reject:border-rose-300">
                                    点击查看
                                  </span>
                                </button>
                              </PopoverTrigger>
                      ...
                      <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-end gap-2 flex-wrap">
                        <button
                          onClick={() => navigate(`/hotels/${h.id}`)}
                          className="px-3 py-1.5 text-[13px] font-semibold rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-200 transition-colors"
                        >
                          详情
                        </button>
                        ...
                        {isMerchant && (
                          <button
                            onClick={() => navigate(`/hotels/${h.id}/edit`)}
                            className="px-3 py-1.5 text-[13px] font-semibold rounded-lg bg-blue-50 text-blue-700 hover:bg-blue-100 transition-colors"
                          >
                            编辑
                          </button>
                        )}
                        ...
                        {isAdmin && Number(h.audit_status) === 0 && (
                          <>
                            <button
                              onClick={() => triggerAudit(h.id, "approve")}
                              className="px-3 py-1.5 text-[13px] font-semibold rounded-lg bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors"
                            >
                              批准
                            </button>
                            <button
                              onClick={() => triggerAudit(h.id, "reject")}
                              className="px-3 py-1.5 text-[13px] font-semibold rounded-lg bg-rose-50 text-rose-700 hover:bg-rose-100 transition-colors"
                            >
                              驳回
                            </button>
                          </>
                        ...
                        {isAdmin &&
                          Number(h.audit_status) === 1 &&
                          Number(h.online_status) === 0 && (
                            <button
                              onClick={() =>
                                handleAction(() => publishHotel(h.id))
                              }
                              className="px-3 py-1.5 text-[13px] font-semibold rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors shadow-sm shadow-blue-500/20"
                            >
                              上线
                            </button>
                          )}
                        ...
                        {(Number(h.online_status) === 1 || isMerchant) && (
                          <button
                            onClick={() => confirmOffline(h.id)}
                            className="px-3 py-1.5 text-[13px] font-semibold rounded-lg text-slate-500 hover:text-rose-600 hover:bg-rose-50 transition-colors"
                          >
                            下线
                          </button>
                        )}
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-6 mt-12 mb-6">
                ...
                <span className="text-sm font-medium text-slate-500 bg-slate-100 px-4 py-1.5 rounded-lg">
                  第{" "}
                  <span className="text-slate-900 font-bold mx-0.5">
                    {currentPage}
                  </span>{" "}
                  / {totalPages} 页
                </span>
