...
function getStatusMeta(hotel) {
  ...
  if (audit === 1 && online === 0)
    return { label: "待上线", className: "approvedOffline" };
  if (audit === 1 && online === 1)
    return { label: "已上线", className: "online" };
    ...
    <div className="dashboard-page">
      <div className="top-nav">
        <div className="nav-logo">
          ...
          <span>易宿平台 · {isAdmin ? "管理员工作台" : "商家工作台"}</span>
        </div>
        <div className="user-info">
          <span>
            {user?.name || user?.account}（{isAdmin ? "管理员" : "商家"}）
          </span>
      ...
      <div className="container">
        ...
        <div className="search-row">
          ...
          <button className="btn-add" onClick={loadHotels}>
            {loading ? "查询中..." : "查询"}
          </button>
        </div>
        ...
        <div className="hotel-list">
          {hotels.map((h) => (
            <div className="hotel-card" key={h.id}>
              <div className="card-thumb">
                {cover(h) ? (
                  <img
                    ...
                    onError={(e) => {
                      e.currentTarget.src =
                        "https://picsum.photos/seed/yisu_web_fallback/1280/720";
                    }}
                  />
                ) : (
              ...
              <div className="hotel-attr">
                {h.starLevel}星 · {h.city}
                {h.county}
              </div>
              <div className="hotel-attr">{h.address}</div>
              <div className={`status-chip ${getStatusMeta(h).className}`}>
                {getStatusMeta(h).label}
              </div>
              {Number(h.audit_status) === 2 && h.audit_reason ? (
                <div
                  className="hotel-attr"
              ...
              <div className="card-btn-group">
                ...
                {isAdmin && Number(h.audit_status) === 0 ? (
                  <>
                    <button
                      ...
                      onClick={() =>
                        handleAction(() => auditHotel(h.id, "approve"))
                      }
                    >
                      审核通过
                    </button>
                    <button
                      ...
                      onClick={() =>
                        handleAction(() => auditHotel(h.id, "reject"))
                      }
                    >
                      驳回
                    </button>
                ...
                Number(h.online_status) === 0 ? (
                  <button
                    className="action-btn approve"
                    onClick={() => handleAction(() => publishHotel(h.id))}
                  >
                    发布上线
                  </button>
                ) : null}
                ...
                {Number(h.online_status) === 1 || isMerchant ? (
                  <button
                    ...
                    onClick={() => handleAction(() => offlineHotel(h.id))}
                  >
                    下线
                  </button>
                ) : null}
          ...
          {!hotels.length && !loading ? (
            <div className="empty-tip">暂无酒店数据</div>
          ) : null}
        </div>
      </div>
    </div>
