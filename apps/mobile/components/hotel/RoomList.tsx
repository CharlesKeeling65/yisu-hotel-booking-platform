      ...
      <View className="ml-3 flex-1">
        <View className="flex-row items-start justify-between">
          <View className="min-w-0 flex-1 pr-2">
            <Text className="font-semibold text-neutral-900" style={{ fontSize: nameSize }} numberOfLines={1}>
              {room.name}
            </Text>
            <Text className="mt-1 text-neutral-500" style={{ fontSize: metaSize }} numberOfLines={1}>
              {room.bedType} · 可住 {room.capacity} 人 · {areaText}
            </Text>
            <Text className="mt-1.5 text-neutral-500" style={{ fontSize: metaSize }} numberOfLines={1}>
              {room.breakfastIncluded ? '含早餐' : '不含早餐'} · {room.refundable ? '可取消' : '不可取消'}
            </Text>
          </View>
        </View>
        ...
        <View className="mt-1.5 flex-row items-end justify-end">
          <View className="flex-row items-end gap-2.5">
            <Text>
              <Text className="font-semibold text-amber-500" style={{ fontSize: Math.max(13, Math.floor(priceSize * 0.54)) }}>
                ¥
              </Text>
              <Text className="font-semibold text-amber-600" style={{ fontSize: priceSize - 1 }}>
                {room.price}
              </Text>
            </Text>
            <Pressable disabled={!available} className={`rounded-xl border px-3 py-1.5 ${available ? 'border-[#B7DEFF] bg-[#2B7FC7]' : 'border-slate-200 bg-slate-100'}`}>
              <Text className={`font-medium ${available ? 'text-[#fff]' : 'text-slate-400'}`} style={{ fontSize: actionSize }}>
                订
              </Text>
            </Pressable>
          </View>
