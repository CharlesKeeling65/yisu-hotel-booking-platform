...
import PriceStarPickerSheet, {
    PriceStarDraft,
} from "@/components/hotel/PriceStarPickerSheet";
...
const DEFAULT_PRICE_STAR_FILTER: PriceStarDraft = {
  min: 0,
  max: 800,
  stars: [],
};
...
function parsePriceStarLabel(label: string): PriceStarDraft {
  ...
  if (range?.[1] && range?.[2])
    return { min: Number(range[1]), max: Number(range[2]), stars };
...
export default function HomeScreen() {
  ...
  const [city, setCity] = useState(
    stripCityCountySuffix(initSession.city || "上海市"),
  );
  const [location, setLocation] = useState(initSession.location || "");
  const [checkInDate, setCheckInDate] = useState(
    initSession.checkIn || defaultSession.checkIn,
  );
  const [checkOutDate, setCheckOutDate] = useState(
    initSession.checkOut || defaultSession.checkOut,
  );
  const [rooms, setRooms] = useState(
    Math.max(1, Number(initSession.rooms || 1)),
  );
  const [adults, setAdults] = useState(
    Math.max(
      Number(initSession.adults || 1),
      Math.max(1, Number(initSession.rooms || 1)),
    ),
  );
  const [children, setChildren] = useState(
    Math.max(0, Number(initSession.children || 0)),
  );
  const [priceStarFilter, setPriceStarFilter] = useState<PriceStarDraft>(
    parsePriceStarLabel(initSession.priceStar || ""),
  );
  ...
  const [banners, setBanners] = useState<HomeBanner[]>([
    {
      ...
      title: "易宿精选酒店",
      subtitle: "品质酒店限时优惠",
      image: "https://picsum.photos/seed/home_banner/1400/700",
    },
  ]);
  ...
  useEffect(() => {
    if (typeof cityParam === "string")
      setCity(stripCityCountySuffix(cityParam));
  }, [cityParam]);
    ...
    useCallback(() => {
      ...
      setAdults(
        Math.max(
          ...
          Math.max(1, Number(current.rooms || 1)),
        ),
      );
    ...
    >
      ...
      <View className="mt-4">
        <SearchPanel
          ...
          onLocationPress={() =>
            router.push({ pathname: "/location", params: { city, location } })
          }
