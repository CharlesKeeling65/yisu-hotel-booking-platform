...
async function queryHotels(req) {
  ...
  // 【修复 Bug 11】：将严格的 "=" 改为 "LIKE"，解决“广州”查不到“广州市”的问题
  if (q.city) {
    ...
    const cityLike = `%${cityRaw}%`;
    params.push(cityLike, cityLike, cityLike);
  }

  // 【修复 Bug 12】：精简关键字匹配，去掉会导致串台的标签联表匹配，增加对简介的匹配
  if (q.keyword) {
    where.push(`(
      b.name_cn LIKE ?
      OR b.name_en LIKE ?
  ...
  // 【修复 Feature 11】：增加 LEFT JOIN User u 表，获取商家的 name 和 email
  const [rows] = await pool.query(
    `SELECT b.*, a.audit_status, a.online_status, a.status, a.audit_reason, a.auditor_id, a.audit_time, a.online_time,
